...
function createPost(){
    ...
    if(postTitle.value === "" || uploadedFile.files.length == 0 || (uploadedFile.files[0].type !== "audio/wav" || uploadedFile.files[0].type !== "audio/mpeg"|| uploadedFile.files[0].type != "video/webm")){ //If there's no title or file, or file is mp3, then it doesn't actually create a post (body text is optional)
